...
const ROWS = [
  ...
  { to: '/settings', icon: 'settings', label: 'Settings', sub: 'Appearance and app preferences' },
]
  ...
  if (!user) return (
    <div style={{ paddingTop: 64, minHeight: '100vh', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '48px 20px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: 16, marginBottom: 28 }}>Sign in to manage bookings, track your cleaner, and more.</p>
        ...
        <Link to="/settings" style={{
          display: 'flex', alignItems: 'center', gap: 12, marginTop: 28, padding: '14px 4px',
          borderTop: '1px solid var(--border)', color: 'var(--text)', fontWeight: 600, fontSize: 15
        }}>
          <Icon name="settings" size={19} color="var(--text-muted)" /> Settings
        </Link>
      </div>
    </div>
  )
    ...
    <div style={{ paddingTop: 64, minHeight: '100vh', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {ROWS.map(row => (
            <Link key={row.to} to={row.to} style={{
              ...
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--tile)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Icon name={row.icon} size={20} color="var(--text)" />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>{row.label}</div>
                <div style={{ fontSize: 13, color: 'var(--text-dim)', marginTop: 2 }}>{row.sub}</div>
              </div>
        ...
        <button onClick={handleSignOut} style={{
          ...
          background: 'var(--tile)', color: '#E11900', fontSize: 15, fontWeight: 700
        }}>
          Sign Out
        </button>
      </div>
