...
export default function BookingSuccess() {
  ...
  return (
    <div style={{ paddingTop: 68, minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '100px 24px 60px' }}>
      <div style={{ maxWidth: 500, width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'inline-block', background: 'rgba(0,200,150,0.1)', border: '1px solid rgba(0,200,150,0.25)', borderRadius: 12, padding: '12px 28px', marginBottom: 32 }}>
          <div style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 4 }}>Booking Reference</div>
          <div style={{ color: '#00C896', fontWeight: 800, fontFamily: 'Inter', fontSize: 20 }}>{bookingId}</div>
        </div>

        {/* Booking details if loaded */}
        {booking && (
          <div style={{ background: 'var(--tile)', border: '1px solid var(--border)', borderRadius: 16, padding: 24, textAlign: 'left', marginBottom: 32 }}>
            {[
              ...
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--tile-2)' }}>
                <span style={{ color: 'var(--text-muted)', fontSize: 14 }}>{row.label}</span>
                <span style={{ color: row.green ? '#00C896' : 'var(--text)', fontSize: 14, fontWeight: row.green ? 700 : 500 }}>{row.value}</span>
              </div>
            ))}
          </div>
        )}
